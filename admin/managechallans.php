<?php
	...
	$studData = sqlgetresult('SELECT c.*,s."studentName", s."section",s."hostel_need" FROM tbl_temp_challans c LEFT JOIN tbl_student s ON s."studentId" = c."studentId"  WHERE c."feeType" IS  NULL ORDER BY s."studentName" ASC',true);
	 // print_r($studData);
?>
...
<div class="container-fluid">
	<div class="row col-md-12">
         ...
         <div class="col-xs-10 col-sm-8 col-md-6 col-lg-4">  
            <?php
               ...
               } elseif(isset($_SESSION['successdelete'])) {
   ...
   <h2 class="top">TEMPORARY CHALLANS</h2>
     ...
     <div class="col-sm-9 p-r-0 text-right">
		<form method="post" id = "filterchallan" class="form-inline" action="adminactions.php">
            <div class="form-group">
                 <div>
                    <select id="transporttype"  name="transporttype"  class="transporttype form-control">
                      <option value="">Select</option>
                      <?php
                        $transportStages = sqlgetresult("SELECT id , feename FROM getfeetypedata WHERE feename ilike '%stage%' GROUP BY id , feename ");
                        foreach ($transportStages as $k=>$v) {
                            $id = $k+1;
                            echo "<option value = '".$id."'>".$v['feename']."</option>";
                        }
                      ?>
                    </select>
                  </div>
             ...
             <div class="form-group">
             <div>
                <select id="studdatatype" name="studdatatype"  class="studdatatype form-control">
                  <option value="">Select</option>
            ...
            <div class="form-group">
                 <div>
                    <select id="studtype" name="studtype"  class="studtype form-control">
            ...
            <div class="form-group">
                ...
                <div class="form-group">
                  ...
                  <div>
                    ...
                    ?>
                    <select id="sectionselectt" name="sectionselectt"  class="sectionselect  form-control">
                      <option value="">Section</option>                     
                    </select>
                  </div>                                    
                </div>
                <button type="button" id="fltchallan" name="filter" value="filterchallan" class="btn btn-info">Filter</button>
              </form>
              </div>
           </div>
...
<div class="table-responsive">
    <input type="hidden" name="cidd" id="cidd" />
		 <table class="table table-bordered dataTableChallan admintab">
          ...
          if($num > 0)
          {
				...
				<tr class="filtertempchallan">
					    <td><input type="checkbox" name="checkme[]" class="checkme" id="checkme<?php echo $k;?>" value="<?php echo $data["studentId"];?>" style="margin:10px;"><input type="hidden" name="cno" class="cno"  value="<?php echo $data["challanNo"];?>"></td>
						<td><?php echo $i;?></td>
            ...
            <td><?php echo $data['hostel_need'];?></td>
						<td class="fafa">
					    	<a href="editStudent.php?id=<?php echo trim($data["studentId"]); ?>&cid=<?php echo trim($data["challanNo"]); ?>"><i class="fa fa-edit"></i></a>
                <a href="adminactions.php?actions=deletetemp&id=<?php echo $data["challanNo"]; ?>" onclick="return confirmDelete('Challan. If so you have to re-create the temporary challan for the following Student Id ')"><i class="fa fa-trash-o"></i></a>
					    </td>
