...
if(isset($_GET['id']) && !empty($_GET['id'])){
    ...
    array_push($className, getClassbyNameId($st_class));
    array_push($semester, $st_sem);
}else if(isset($_SESSION['selectedtransfeechallans'])) {
    $type='bulk';
    $selectedchallan=$_SESSION['selectedtransfeechallans'];
   ...
   <div class="row">
      ...
      <div class="col-sm-8 col-md-6">
         <div class="tech-content">
            <p class="heading">Create Transport Fee Challan</p>          
               <form id="feechallanform" method="post" class="form-horizontal" onsubmit="return changeAction();" action="adminactions.php">
                  ...
                  <input name="semester" id="semester" type="hidden" value="<?php echo $st_sem;?>">
                    ...
                    <div id='demandData' class="feetab1 edittech">
                        <div class="form-group">
                            ...
                            <div class="col-sm-8">
                                <input type="text" class="duedate datepicker form-control" name="duedate"  value="<?php echo date("Y-m-d"); ?>" required>
                            </div>
                        </div>
                        <div class="form-group">
                ...
                <?php
                    foreach($semestercheck as $semester) {
                        $semster=trim($semester['semester']);
                        if($semster != $sem){
                            $slcted="selected='selected'";
                        }else{
                            $slcted="";
                        }
                        echo '<option value="'.$semster.'" '. $slcted.' >'.$semster.'</option>';
                    }
                ?>
                </select>
                            </div>                                    
                        ...
                        <div class="form-group text-center"> 
                            <input type="hidden" name="showtransfeechallanFrmPaid" value="showtransfeechallanFrmPaid" >                         
                            <button type="submit" name="showtransfeechallanFrmPaid" value="showtransfeechallanFrmPaid" class="btn btn-primary text-center" id="showtransfeechallanFrmPaid">Create Fee Challan</button>
                            <button type="button" value="back" name="back" class="btn btn-warning text-center" onclick="window.location.href='managetransportreport.php'">Back</button>
                        </div>
                    </div>  
                </form>                  
             </div>
