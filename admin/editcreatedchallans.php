...
$id=$_REQUEST['id'];
$query = 'SELECT c.*, s."studentName", s."hostel_need" FROM tbl_challans c LEFT JOIN tbl_student s ON c."studentId" = s."studentId"  WHERE "challanNo"=\'' . $id . '\' ';
$res = sqlgetresult($query,true);
// print_r($res);
// exit;
$feeData=array();
foreach($res as $result){
  $fee=explode(",",$result['feeType']);
  foreach ($fee as $v) {
    $feeData[] = $v;
  }
                    ...
                    <div class="form-group">
                      ...
                      <div id='demandData' class="feetab1">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-8">
                                      <input type="hidden" name="oldfeetype" id="oldfeetype" value="<?php echo $res[0]['feeType'];?>">
                                         <?php
                                            if( $res[0]['hostel_need'] == 'Y') {
                                                $feeTypes = sqlgetresult('SELECT * FROM getfeetypes WHERE (applicable=\'DH\' OR applicable=\'H\' OR applicable ILIKE \'%T%\') AND "feeGroup" <> \'10\'',true);
                                                // echo('SELECT * FROM getfeetypes WHERE (applicable=\'DH\' OR applicable=\'H\') AND "feeGroup" <> \'10\'');
                                            } else {
                                                $feeTypes = sqlgetresult('SELECT * FROM getfeetypes WHERE (applicable=\'DH\' OR applicable=\'D\' OR applicable ILIKE \'%T%\') AND "feeGroup" <> \'10\'',true);
                                                // echo('SELECT * FROM getfeetypes WHERE (applicable=\'DH\' OR applicable=\'D\') AND "feeGroup" <> \'10\'');
                                            }
                                                    ...
                                                    if (stripos($res[0]['studentId'], 'APPL') !== false) {
                                                        // if(contains(trim($feetype['feeType']), $newstudent_feetypes)) {
                                                          // if($feetype['mandatory'] == 1){
                                                            if( in_array(trim($feetype['id']),$selectedfeetypes) ) {
                                ...
                                <div class="form-group ">
                                        ...
                                        <div class="col-sm-8">
                                            <textarea placeholder="REMARKS" class="form-control remarks" name="remarks" maxlength="250"><?php echo trim($res[0]['remarks']);?></textarea>
                                        </div>
                                </div>
                                <div class="form-group text-center">
                                   
                                    <input type="hidden" name="editcreatedchallans" value="editcreatedchallans" >
                                    <button type="submit" name="editcreatedchallans" value="editcreatedchallans" class="btn btn-primary text-center" id="editcreatedchallans">Update Challan</button>
