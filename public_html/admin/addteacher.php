				...
				<div class="main">
					...
					<form id="user_registered" method="post" action="adminactions.php">
						...
						<div class="form-group">
								...
								<!-- <input type="text" name="class" required placeholder="Class" class="form-control"> -->
						</div>		
						<div class="form-group">
                            <label class="control-label" for="studStatus">Class</label>
                                 <?php
                                    $feeTypes = sqlgetresult("SELECT * FROM classcheck WHERE status =  'ACTIVE' ");
                                ?>
                                <input type="hidden" name="selected_class" class="selected_quizsetids">
                                <select name="class"  class="quizsetid form-control" multiple="multiple" required>
                                    <?php
                                            foreach($feeTypes as $class) {
