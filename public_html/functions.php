...
CREATE OR REPLACE FUNCTION reg_val(fname character(20),lname character(20),un character(30),e character(50),pn1 bigint,pn2 bigint,pw character(150),vc character(15))
  RETURNS int AS $$
DECLARE exist int;
BEGIN

    PERFORM  1 FROM public.tbl_parents WHERE email = e ;      

      IF NOT found THEN
         INSERT INTO public.tbl_parents("firstName", "lastName", "userName", "email", "phoneNumber", "mobileNumber", "password", "verifyCode", "createdOn") VALUES(fname,lname,un,e,pn1,pn2,pw,vc,CURRENT_TIMESTAMP);
         exist = 0;
       ELSE 
         exist = 1;
      END IF;  
...
CREATE OR REPLACE FUNCTION add_stud(pid integer,mob character(30),sid character(10))
  ...
  DECLARE fid int;
BEGIN
  PERFORM  1 FROM public.tbl_student WHERE "parentId" = pid AND "studentId"=sid ;      

    IF NOT found THEN
     UPDATE public.tbl_student SET "parentId"=pid , "updatedOn"=CURRENT_TIMESTAMP WHERE "studentId" = sid AND "mobileNumber" = mob RETURNING id INTO fid ;        
  ELSE 
    fid = 0;
  END IF;
  return fid; 
...
BEGIN
    ...
    PERFORM  1 FROM public.tbl_admin WHERE "adminEmail" = adme ;      
...
CREATE OR REPLACE FUNCTION editpar(did integer, fn character(20),ln character(20),un character(40), e character(30),p character(20),pn bigint,mn bigint,uid integer) 
  RETURNS int AS $$
BEGIN
    PERFORM  1 FROM public.tbl_parents WHERE "email" = e AND "id"!=did;      

    IF NOT found THEN
        UPDATE tbl_parents SET "firstName"=fn,"lastName"=ln,"userName"=un,"email"=e,"password"=p,"phoneNumber"=pn,"mobileNumber"=mn,"updatedOn"=CURRENT_TIMESTAMP,"updatedBy"=uid WHERE "id" = did;
...
CREATE OR REPLACE FUNCTION addpar(fn character(20),ln character(20),un character(40), e character(30),p character(20),pn bigint,mn bigint, uid integer) 
  RETURNS int AS $$
BEGIN
    ...
    IF NOT found THEN
       INSERT INTO public.tbl_parents("firstName", "lastName", "userName","email", "password","phoneNumber","mobileNumber","createdOn","createdBy") VALUES (fn,ln,un,e,p,pn,mn,CURRENT_TIMESTAMP,uid);
...
CREATE OR REPLACE FUNCTION editstd(sid character(10),sn character(20),s character(15),c character(20),sec character(40), t character(30),pi integer,mail character(150),mnum character(30),uid integer,did integer) 
  RETURNS int AS $$
BEGIN
    PERFORM  1 FROM public.tbl_student WHERE "studentId"=sid AND "id"!=did;      

    IF NOT found THEN
        UPDATE tbl_student SET "studentId"=sid,"studentName"=sn,"stream"=s,"class"=c,"section"=sec,"term"=t,"parentId"=pi,"email"=mail, "mobileNumber"=mnum,"updatedOn"=CURRENT_TIMESTAMP,"updatedBy"=uid WHERE "id" = did;
...
CREATE OR REPLACE FUNCTION addstd(sid character(10),sn character(20),s character(15),c character(20),sec character(40), t character(30),pi integer,mail character(150),mnum character(30),uid integer) 
  RETURNS int AS $$
BEGIN
    PERFORM  1 FROM public.tbl_student WHERE "studentId"=sid;     
...
CREATE OR REPLACE FUNCTION editclass(did integer, cn character(20),des character(20),uid integer) 
  RETURNS int AS $$
BEGIN
    PERFORM  1 FROM public.tbl_class WHERE "class_list" =cn AND "id"!=did;      

    IF NOT found THEN
        UPDATE tbl_class SET "class_list"=cn,"description"=des,"updatedOn"=CURRENT_TIMESTAMP,"updatedBy"=uid WHERE "id" = did;
...
CREATE OR REPLACE FUNCTION addclass(cn character(20),des character(100),uid integer) 
  RETURNS int AS $$
BEGIN
    PERFORM  1 FROM public.tbl_class WHERE "class_list" =cn ;      

    IF NOT found THEN
       INSERT INTO public.tbl_class("class_list", "description","createdOn","createdBy")
       VALUES (cn,des,CURRENT_TIMESTAMP,uid);
...
CREATE OR REPLACE FUNCTION editstream(did integer, s character(20),des character(20),uid integer) 
  RETURNS int AS $$
BEGIN
    PERFORM  1 FROM public.tbl_stream WHERE "stream" =s AND "id"!=did ;      
...
CREATE OR REPLACE FUNCTION addstream(s character(20),des character(100),uid integer) 
  RETURNS int AS $$
BEGIN
    PERFORM  1 FROM public.tbl_stream WHERE "stream" =s ;      
...
CREATE OR REPLACE FUNCTION edityear(did integer,yr character,uid integer) 
  RETURNS int AS $$
BEGIN
    PERFORM  1 FROM public.tbl_academic_year WHERE "year" =yr AND "id"!=did;      
...
CREATE OR REPLACE FUNCTION addyear(yr character,uid integer) 
  RETURNS int AS $$
BEGIN
    PERFORM  1 FROM public.tbl_academic_year WHERE tbl_academic_year."year" = yr ;      
...
CREATE OR REPLACE FUNCTION editcomments(did integer, pn character(20),com text,uid integer) 
  RETURNS int AS $$
BEGIN
    PERFORM  1 FROM public.tbl_comments WHERE "pageName" =pn AND "id"!=did;      

    IF NOT found THEN
        UPDATE tbl_comments SET "pageName"=pn,"comments"=com,"updatedOn"=CURRENT_TIMESTAMP,"updatedBy"=uid WHERE "id" = did;
...
CREATE OR REPLACE FUNCTION addcomments(pn character(20),com text,uid integer) 
  RETURNS int AS $$
BEGIN
    PERFORM  1 FROM public.tbl_comments WHERE "pageName" =pn ;      

    IF NOT found THEN
       INSERT INTO public.tbl_comments("pageName","comments","createdOn","createdBy")
       VALUES (pn,com,CURRENT_TIMESTAMP,uid);
...
END;
 ...
 /*****Update Temp Challan - Start******/
 CREATE OR REPLACE FUNCTION createlatefeechallan(cn character(50),studId character(30),classlist character(5),trm character(5),latefee real,str integer,due date,yr character(20))
  RETURNS int AS $$ 
    DECLARE cid int;
BEGIN
    PERFORM 1 FROM public.tbl_challans WHERE "studentId" =studId AND "classList" = classlist  AND "term" = trm  AND "stream" = str;
...
CREATE OR REPLACE FUNCTION cheque_fee_entry_update(fid character(20),uby integer, ptype character(30),bnk character(100), cheque_no character(100),pdate date)
  RETURNS int AS $$
BEGIN
     UPDATE tbl_variable_fee_entry SET "transId" = 0, "updatedBy" = uby, "updatedOn" = CURRENT_TIMESTAMP, "pay_type" = ptype, "bank" = bnk, "cheque_dd_no" = cheque_no, "paid_date" = pdate WHERE id = fid::int;
     return 1;    
END;
...
/****** This view is for to payment Report *****/
 create view getpaiddata AS SELECT p."transStatus",
    ...
    p.id
   FROM tbl_payments p
     JOIN tbl_student s ON p."studentId" = s."studentId"
...
s."stream",p."amount",pr."userName" from tbl_payments p 
...
create view gettempdata AS SELECT t."studentId",t."studentName",c."class_list",t."classList",t."stream", t."term", t."studStatus", str."stream" AS streamname,s."section", t."challanNo",s."hostel_need",s."transport_need" from tbl_temp_challans t left join tbl_student s  ON t."studentId" = s."studentId"
...
CREATE VIEW waviercheck AS SELECT s."studentName",s."section", cl."class_list",st."stream", c."term", c."total", c."studentId",c."challanNo",c."waived",c."org_total", c.id FROM tbl_challans c
LEFT JOIN tbl_student s  ON s."studentId" = c."studentId" 
LEFT JOIN tbl_class cl ON cl.id = c."classList"::int
LEFT JOIN tbl_stream st ON St.id = c."stream"
     WHERE c."deleted" = 0 ORDER BY c."id" ASC
/****** This view is for to waviercheck- End*****/


 CCREATE VIEW waviercheck AS
...
/****** This view is for to getpaiddata- Start*****/
 create view getpaiddata AS 
  SELECT p."transStatus",
    p."transDate",
    p."transNum",
    p."studentId",
    s."studentName",
   ...
   FROM tbl_payments p
     JOIN tbl_student s ON p."studentId" = s."studentId"
     LEFT JOIN tbl_parents par ON p."parentId" = par.id;
/****** This view is for to getpaiddata- End*****/
...
CREATE VIEW parentcheck AS SELECT CASE WHEN "status" = '1' THEN 'ACTIVE'
            ELSE 'INACTIVE'
       END  AS status, id, "firstName", "lastName", "userName", "email", "phoneNumber", "mobileNumber", "password" FROM tbl_parents WHERE "deleted" = 0 ORDER BY "userName" ASC
...
// ***** VIEW FOR FILTERFEEDETAILS FOR FEE CONFIGURATION PAGE - Start*****//

  CREATE VIEW filterstudentdetails AS SELECT CASE WHEN s."status" = '1' THEN 'ACTIVE'
            ELSE 'INACTIVE'
       END  AS status, s.id, s."studentName",s."class", c."class_list",str."stream" AS streamname,s."section", s."studentId",s."term", p."userName",s."stream" FROM tbl_student s 
       left join tbl_class c ON c.id=s."class"::int
       left join tbl_stream str ON str.id = s.stream::int 
       left JOIN tbl_parents p ON s."parentId"  = p.id WHERE s."deleted" = 0 ORDER BY s."studentName" ASC;
...
CREATE VIEW getfeetypes AS SELECT * FROM tbl_fee_type WHERE "deleted" = 0 AND status = '1';

 CREATE VIEW getfeetypes AS SELECT tbl_fee_type.id,
    tbl_fee_type."feeType",
    tbl_fee_type.description,
    tbl_fee_type."createdBy",
    tbl_fee_type."updatedBy",
    tbl_fee_type.status,
    tbl_fee_type.deleted,
    tbl_fee_type."createdOn",
    tbl_fee_type."updatedOn",
    tbl_fee_type.tax,
    tbl_fee_type."feeGroup",
    tbl_fee_type.mandatory,
    tbl_fee_type.applicable
   FROM tbl_fee_type
  WHERE tbl_fee_type.deleted = 0 AND tbl_fee_type.status = '1'::status; 
...
CREATE VIEW classcheck AS SELECT CASE WHEN "status" = '1' THEN 'ACTIVE'
            ELSE 'INACTIVE'
       END  AS status, id, "class_list", "description"  FROM tbl_class WHERE "deleted" = 0 ORDER BY "id" ASC
...
CREATE VIEW feetypecheck AS SELECT
        CASE
            WHEN tbl_fee_type.status = '1'::status THEN 'ACTIVE'::text
            ELSE 'INACTIVE'::text
        END AS status,
    tbl_fee_type.id,
    tbl_fee_type."feeType",
    ...
    tbl_fee_type.applicable
   FROM tbl_fee_type
  WHERE tbl_fee_type.deleted = 0
  ORDER BY tbl_fee_type.id;
...
CREATE VIEW commentscheck AS SELECT CASE WHEN "status" = '1' THEN 'ACTIVE'
            ELSE 'INACTIVE'
       END  AS status, id, "pageName", "comments"  FROM tbl_comments WHERE "deleted" = 0 ORDER BY "id" ASC
...
/***VIEW FOR tempChallan - Start ****/
 create view tempChallan AS SELECT * FROM tbl_temp_challans WHERE "deleted" = 0 AND status = '1' ;
...
CREATE VIEW filterfeedetails AS SELECT f."academicYear",f.id,t."feeType",s.stream,c.class_list,f.amount,f.semester,f."dueDate" FROM tbl_fee_configuration f LEFT JOIN tbl_fee_type t ON f."feeType"::int = t.id LEFT JOIN tbl_stream s ON f.stream::int = s.id LEFT JOIN tbl_class c ON f.class = c.id WHERE f.deleted=0;
// ***** VIEW FOR FILTERFEEDETAILS FOR FEE CONFIGURATION PAGE - End*****//


/***VIEW FOR feeconigdata Year PAGE - Start ****/
CREATE VIEW feeconigdata AS SELECT f."academicYear",f.id,t."feeType",s.stream,c.class_list,f.amount,f.semester,f."dueDate" FROM tbl_fee_configuration f LEFT JOIN tbl_fee_type t ON f."feeType"::int = t.id LEFT JOIN tbl_stream s ON f.stream::int = s.id LEFT JOIN tbl_class c ON f.class::int = c.id WHERE f.deleted=0;
...
CREATE VIEW addsemesterdata as select * from tbl_ semester 
...
/***VIEW FOR challandata - Start ****/
// create view challandata AS   SELECT s."studentId",
//     s."studentName",
//     cl.class_list,
//     cl.id AS clid,
//     s.section,
...
//     s."parentId",
//     s.stream,
//     str.stream AS steamname,c."dudate"
//    FROM tbl_student s
//      LEFT JOIN tbl_challans c ON s."studentId" = c."studentId"
//      LEFT JOIN tbl_semester sem ON sem.id::character(1) = c.term
//      LEFT JOIN tbl_class cl ON c."classList"::integer = cl.id
...
/***Alter for tbl_teachers_id_seq - Start ****/
 ALTER SEQUENCE tbl_teachers_id_seq RESTART WITH 1;
...
c."challanNo",f."feeType",cl.class_list,c."duedate"
 ...
 CREATE VIEW challandatanew ASSELECT DISTINCT c."feeTypes",
    s."studentId",
    s."studentName",
    cl.class_list,
    cl.id AS clid,
    ...
    c."waivedPercentage",
    c."waivedAmount",
    c."waivedTotal",s.academic_yr
   FROM tbl_student s
     LEFT JOIN tbl_challans c ON s."studentId" = c."studentId"
     LEFT JOIN tbl_semester sem ON sem.id::character(1) = c.term
     LEFT JOIN tbl_class cl ON c."classList"::integer = cl.id
     LEFT JOIN tbl_stream str ON s.stream::integer = str.id
     LEFT JOIN tbl_variable_fee_entry f ON c."challanNo" = f."challanNo"
  WHERE c.deleted = 0 AND c.status = '1'::status;

  CREATE VIEW getchallandata AS  SELECT c."challanNo",
    SELECT c."challanNo",
    c."classList",
    c.stream,
    c.term,
...
CREATE OR REPLACE FUNCTION createlatefeechallan(cn character(50),studId character(30),classlist character(5),trm character(5),latefee real,str integer,due date)
  RETURNS int AS $$ 
    DECLARE cid int;
BEGIN
    PERFORM 1 FROM public.tbl_challans WHERE "studentId" =studId AND "classList" = classlist  AND "term" = trm  AND "stream" = str;
    IF found THEN
      INSERT INTO public.tbl_challans("challanNo","studentId","classList", "term","org_total", "stream","studStatus","chalanStatus","createdOn","duedate","feeGroup") VALUES (cn, classlist,trm,latefee,str,'Prov.Promoted',0,CURRENT_TIMESTAMP,due,'LATE FEE') RETURNING id INTO cid;    
    ELSE 
      cid = 0;
    END IF;
    return cid;
